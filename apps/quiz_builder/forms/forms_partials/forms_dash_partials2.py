...
def create_modal_validation(message_modal, id_modal_message):
    modal_content =  html.Div(
        [
            html.Div(style={'height': '100px', 'pading-left':'300px'}),
                    ...
                    dbc.ModalFooter(
                        [
                            dbc.Button(
                                "Cancelar",
                                id="close-centered",
                                className="ms-auto btn-danger",  # Agregar la clase para el color rojo
                                n_clicks=0,
                            ),
                            dbc.Button(
                                "Aceptar",
                                id="aceptar-centered",
                                className="ml-2 btn-success",  # Agregar la clase para el color verde y moverlo a la derecha
                                n_clicks=0,
                            ),
                        ]
                    ),
...
def input_text(name_field, type_field, id_name_field, disabled=False):
    ...
    return [
        ...
        dbc.Row([
            ...
            dbc.Col((
                dbc.Row([
                    dbc.Col(html.Label(name_field, style={'font-size': '20px'}), md=12, lg=12),
                    dbc.Col(html.Div([
                        dcc.Input(
                            id=f'{id_name_field}',
                            ...
                            value='',
                            style={'width': '100%'},
                            disabled=disabled  # Aquí está la modificación
                        ),
                    ], className='pl-0 d-flex align-items-center'), md=12, lg=12),
                ])
            ), width=8),
            dbc.Col(width=2),
        ]),
...
def input_dropdown_field(name_dropdown, list_value_dropdown_field, id_dropdown_field):
    return dbc.Row([
        ...
        dbc.Col(width=2),
        dbc.Col(html.Div([
            html.Label(name_dropdown, style={'font-size': '20px'}),
            dcc.Dropdown(
...
def input_text_area(input_title_text_area, id_input_text_area):
    ...
    return [
        ...
        dbc.Row([                
            ...
            dbc.Col((
                dbc.Row([
                    dbc.Col(html.Label(input_title_text_area, style={'font-size': '20px'}), md=12, lg=12),
                    dbc.Col(
                        (html.Div([
                            dcc.Textarea(
    ...
    if list_of_contents is not None:
        ...
        for file in data:
            ...
            if file['File Size'] > 5242880:
                return dash.no_update, message_alert('Archivo excede tamaño máximo')
